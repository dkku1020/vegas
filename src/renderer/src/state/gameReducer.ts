...
export const TABLE_MAX_BET = 20000;
  ...
  switch (action.type) {
    case "PLACE_BET": {
      ...
      return {
        ...
        bets: {
          ...state.bets,
          [action.spot]: state.bets[action.spot] + action.amount,
        },
      };
    }
    case "CLEAR_BETS": {
      if (state.phase !== "betting") return state;
      const totalReturned =
        state.bets.player + state.bets.banker + state.bets.tie;
      return {
        ...state,
    ...
    case "DEAL": {
      ...
      const totalCredited =
        ...
        settlement.payouts.banker +
        settlement.payouts.tie;
